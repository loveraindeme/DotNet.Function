...
namespace DotNet.EFCore.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        {
            var user = await _userRepository.GetAsync(input.Id) ?? throw new Exception("User not found");
            user.Age = input.Age;
        ...
        {
            await _userRepository.RemoveAsync(ids);
            await _unitOfWork.SaveChangesAsync();
        }

        [HttpGet("deleted")]
        public async Task<List<UserDto>> GetDeletedListAsync()
        {
